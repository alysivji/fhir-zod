// Source: https://hl7.org/fhir/STU3/codesystem-definitions.html#CodeSystem.concept.property
    ...
    .superRefine((value, ctx) => {
      ...
      if (value_x_Present.length === 0) {
        ctx.addIssue({
          ...
          message:
            "One of valueBoolean, valueCode, valueCoding, valueDateTime, valueInteger, valueString must be present for value[x]",
          path: ["valueBoolean"],
        });
      }
      if (value_x_Present.length > 1) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            "Only one of valueBoolean, valueCode, valueCoding, valueDateTime, valueInteger, valueString may be present for value[x]",
