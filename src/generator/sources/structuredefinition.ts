...
import type { FhirRelease } from "../versions.ts";
...
export function buildStructureDefinitionsFromSpec(
	...
	const desiredNames = expandScopeNames(
		options.scopeNames,
		index,
		options.release,
	);
...
function expandScopeNames(
	...
	while (queue.length > 0) {
		...
		for (const dependency of collectReferencedDefinitionNames(
			name,
			index,
			release,
		)) {
...
function collectReferencedDefinitionNames(
	...
	if (name.includes("_")) {
		...
		for (const element of directChildElements(elements, fhirPath)) {
			for (const reference of collectElementTypeRefs(
				element,
				elements,
				release,
			)) {
		...
		if (
			directTypeCode &&
			release.nestedBackboneTypeCodes.includes(directTypeCode)
		) {
...
function buildDefinitionByName(
	...
	return buildRootDefinition(name, scopeNames, index, terminology, release);
}
...
function buildSyntheticDefinition(
	...
	return {
		baseName: inferSyntheticBaseName(rootElement, elements, release),
		description:
...
function inferSyntheticBaseName(
	...
	const normalizedType = normalizeElementType(element, elements, release);
...
function normalizeElementType(
	...
	const firstType = normalizeStructureType(
		element,
		types[0],
		elements,
		release,
...
function normalizeStructureType(
	...
	if (
		release.nestedBackboneTypeCodes.includes(normalizedCode) &&
