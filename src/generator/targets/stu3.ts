...
export type STU3TargetCategory =
	| "abstract-whitelist"
	| "core-resource"
	| "other"
	| "profile-resource";

export type STU3TargetEntry = {
	abstract: boolean;
	baseDefinition: string | null;
	category: STU3TargetCategory;
	kind: string | null;
	name: string;
...
export const stu3AbstractTargetNames = [
	"BackboneElement",
	"DomainResource",
	"Element",
	"Resource",
...
const abstractGenerationWhitelist = new Set<string>(stu3AbstractTargetNames);

export function loadSTU3TargetEntries(): STU3TargetEntry[] {
	const packageRoot = resolveRequiredSpecPackageRoot("stu3");
...
export function summarizeSTU3Targets(entries = loadSTU3TargetEntries()): {
...
function classifyTargetEntry(definition: StructureDefinition): STU3TargetEntry {
	...
	const category: STU3TargetEntry["category"] = isWhitelistedAbstract
		? "abstract-whitelist"
		: isCoreResource
			? "core-resource"
			: isProfileResource
