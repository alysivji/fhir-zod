...
The raw package contents are downloaded on demand into `.local/` so the repository stays small while generation remains pinned and reproducible. The `.context/` directory is reserved for agent and collaboration material.
...
- The extracted package contents live outside git at `.local/spec-cache/<version>/package/`.
...
- Do not hand-edit files inside `.local/spec-cache/`.
...
2. Confirm `.local/spec-cache/<version>/package/StructureDefinition-*.json` exists.
3. Confirm a representative resource such as `.local/spec-cache/<version>/package/StructureDefinition-Patient.json` is present.
