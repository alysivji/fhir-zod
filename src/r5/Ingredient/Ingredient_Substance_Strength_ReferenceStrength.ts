// Source: https://hl7.org/fhir/R5/ingredient-definitions.html#Ingredient.substance.strength.referenceStrength
...
export const Ingredient_Substance_Strength_ReferenceStrengthSchemaInternal =
  BackboneElementSchemaInternal.extend({
    ...
    substance: z.lazy(getCodeableReferenceSchema),
  })
    .strict()
    .superRefine((value, ctx) => {
      ...
      if (strength_x_Present.length === 0) {
        ctx.addIssue({
          ...
          message:
            "One of strengthQuantity, strengthRatio, strengthRatioRange must be present for strength[x]",
          path: ["strengthQuantity"],
        });
      }
      if (strength_x_Present.length > 1) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            "Only one of strengthQuantity, strengthRatio, strengthRatioRange may be present for strength[x]",
