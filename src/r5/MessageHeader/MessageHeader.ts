// Source: https://hl7.org/fhir/R5/messageheader.html
...
export const MessageHeaderSchemaInternal = z
  .object({
    ...
    eventCanonical: fhirCanonical().optional(),
    _eventCanonical: z.lazy(getElementSchema).optional(),
  ...
  .superRefine((value, ctx) => {
    ...
    const event_x_Present = ["eventCanonical", "eventCoding"].filter(
    ...
    if (event_x_Present.length === 0) {
      ctx.addIssue({
        ...
        message:
          "One of eventCanonical, eventCoding must be present for event[x]",
        path: ["eventCanonical"],
      });
    }
    if (event_x_Present.length > 1) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "Only one of eventCanonical, eventCoding may be present for event[x]",
