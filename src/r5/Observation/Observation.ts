// Source: https://hl7.org/fhir/R5/observation.html
...
import type { Observation_TriggeredBy } from "./Observation_TriggeredBy";
import { Observation_TriggeredBySchemaInternal } from "./Observation_TriggeredBy";
...
export interface Observation extends DomainResource {
  ...
  triggeredBy?: Array<Observation_TriggeredBy>;
...
const getObservation_TriggeredBySchema =
  (): z.ZodType<Observation_TriggeredBy> =>
...
export const ObservationSchemaInternal = z
  .object({
    ...
    instantiatesReference: z.lazy(getReferenceSchema).optional(),
    ...
    triggeredBy: z.lazy(getObservation_TriggeredBySchema).array().optional(),
  ...
  .superRefine((value, ctx) => {
    ...
    const instantiates_x_Present = [
      "instantiatesCanonical",
      "instantiatesReference",
    ].filter((field) => record[field] !== undefined);
    if (instantiates_x_Present.length > 1) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "Only one of instantiatesCanonical, instantiatesReference may be present for instantiates[x]",
        path: [instantiates_x_Present[0]],
      });
    }
    ...
    if (value_x_Present.length > 1) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "Only one of valueAttachment, valueBoolean, valueCodeableConcept, valueDateTime, valueInteger, valuePeriod, valueQuantity, valueRange, valueRatio, valueReference, valueSampledData, valueString, valueTime may be present for value[x]",
