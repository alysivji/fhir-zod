...
import { r5ExampleSkips } from "./r5-example-skips.ts";

const fixturesRoot = resolve(process.cwd(), "tests", "fixtures", "r5");
...
function schemaForResourceType(resourceType: string): unknown {
	...
	return Object.entries(r5Schemas).find(([name]) => name === exportName)?.[1];
}
...
describe("R5 official examples", () => {
	for (const resourceName of readdirSync(fixturesRoot).sort()) {
		...
		for (const entry of entries.sort((left, right) =>
			...
			const skipReason = r5ExampleSkips.get(fixtureKey);
