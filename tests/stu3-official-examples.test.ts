...
const fixturesRoot = resolve(process.cwd(), "tests", "fixtures", "stu3");
...
function schemaForResourceType(resourceType: string): unknown {
	...
	return Object.entries(stu3Schemas).find(([name]) => name === exportName)?.[1];
}
...
describe("STU3 official examples", () => {
	for (const resourceName of readdirSync(fixturesRoot).sort()) {
		...
		for (const entry of entries.sort((left, right) =>
			...
			const expectedFailure = stu3ExampleExpectedFailures.get(fixtureKey);
